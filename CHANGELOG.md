...
- **Reference resolution** - New `--resolve-refs` flag to inline all `$ref` references for better compatibility with tools like OpenAI GPT
  - Available in all commands: `fetch`, `reduce`, and `wizard`
  - Resolves schema, parameter, and response references
  - Handles circular references gracefully
  - Preserves parameter refs in operations (required for validation)
